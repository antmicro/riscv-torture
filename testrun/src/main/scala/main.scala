...
object TestRunner extends App
{
  ...
  def compileAsmToBin(asmFileName: String, use_64bit_opcodes: Boolean): Option[String] = {
    ...
    val arch = if (use_64bit_opcodes) "rv64gcvzfh_zba_zbb" else "rv32gcvzfh_zba_zbb"
  ...
  def runIsaSim(bin: String, debug: Boolean, output: Boolean, dump: Boolean): String =
  {
    ...
    val simISAArgs  =  if (use_64bit_opcodes) Seq("--isa=RV64gcV_zfh_Zvl512b_Zve64d_Zba_Zbb") else Seq("--isa=RV32gcV_zfh_Zvl512b_Zve64d_Zba_Zbb")
