...
object Generator extends App
{
  ...
  def generate(confFile: String, outFileName: String): String =
  {
    ...
    val bitmanip_zbb      = (config.getProperty("torture.generator.bitmanip.zbb", "false").toLowerCase == "true")
    ...
    generate(nseqs, memsize, fprnd, mix, vec, use_amo, use_mul, use_div, bitmanip_zba, bitmanip_zbb, outFileName, segment, loop, loop_size, rv_vmem_unit,rv_vmem_const,rv_vmem_vect,rv_vmem_zvlsseg,rv_vinteger,rv_vfixed,rv_vfloat, rv_vreduce,rv_vmask,rv_vpermute,rv_vamo,rv_wide,rv_narrow, vlen, lmul, sew, nr, nf, mask, multi_config, use_64bit_opcodes)
  }

  def generate(nseqs: Int, memsize: Int, fprnd : Int, mix: Map[String,Int], veccfg: Map[String,String], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, use_zba: Boolean, use_zbb: Boolean, outFileName: String, segment : Boolean, loop : Boolean, loop_size : Int, rv_vmem_unit: Boolean, rv_vmem_const: Boolean, rv_vmem_vect: Boolean,rv_vmem_zvlsseg : Boolean, rv_vinteger: Boolean, rv_vfixed: Boolean, rv_vfloat: Boolean, rv_vreduce: Boolean, rv_vmask: Boolean, rv_vpermute: Boolean, rv_vamo: Boolean , rv_wide: Boolean, rv_narrow: Boolean, vlen: Int, lmul: String, sew: Int, nr: Int, nf: Int, mask:Boolean, multi_config: Boolean, use_64bit_opcodes: Boolean): String =
  {
    ...
    val prog = new Prog(memsize, veccfg, loop, use_64bit_opcodes)
    ProgSeg.cnt = 0
    SeqVec.cnt = 0
    // Added RISC-V Vector functionality
    val s = prog.generate(nseqs, fprnd, mix, veccfg, use_amo, use_mul, use_div, use_zba, use_zbb, segment, loop, loop_size,rv_vmem_unit,rv_vmem_const,rv_vmem_vect,rv_vmem_zvlsseg,rv_vinteger,rv_vfixed,rv_vfloat, rv_vreduce,rv_vmask,rv_vpermute, rv_vamo, rv_wide,rv_narrow,vlen, lmul, sew, nr,nf,mask, multi_config, use_64bit_opcodes)

    val oname = "output/" + outFileName + ".S"
    val fw = new FileWriter(oname)
    fw.write(s)
    fw.close()
    val stats = prog.statistics(nseqs,fprnd,mix,vnseq,vmemsize,vfnum,vecmix,use_amo,use_mul,use_div,use_zba,use_zbb,use_vec)
