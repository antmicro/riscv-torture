...
class Prog(memsize: Int, veccfg: Map[String,String], loop : Boolean, use_64bit_opcodes: Boolean)
{
  ...
  val regstats = new HashMap[String,Int].withDefaultValue(0)
    ...
    ("rvv_vfloat"),("rvv_vreduce"),("rvv_vmask"),("rvv_vpermute"),("rvv_vamo"),("rvv_vmem_zvlsseg"),("rvv_vconfig"),("bitmanip_zba"),("bitmanip_zbb"),("unknown")))
    {
      catstats(cat)=0
  ...
  def sequence_stats(mix: Map[String, Int], vecmix: Map[String, Int], nseqs: Int, vnseq: Int, vfnum: Int): String =
  {
    def seq_lt(seq1: (String, Int), seq2: (String, Int)): Boolean =
    {
      ...
      "bitmanip_zba"->25,"bitmanip_zbb"->26,"unknown"->27).withDefaultValue(100)
  ...
  def instruction_stats(): String =
  {
    def cat_lt(cat1: (String, Int), cat2: (String, Int)): Boolean =
    {
      val cathash = HashMap("alu"->1,"cmp"->2,"branch"->3,"jmp"->4,"jalr"->5,
        ...
        "Hwacha_vfpfma"->20,"Hwacha_vfpcvt"->21,"Hwacha_vsmem"->22,"Hwacha_vshared"->23,"Hwacha_vpred"->24,"Hwacha_vcmp"->25,"Hwacha_vmisc"->26,"rvv_vmem_unit"->27,"rvv_vmem_const"->28,"rvv_vmem_vect"->29,"rvv_vinteger"->30,"rvv_vfixed"->31,"rvv_vfloat"->32,"rvv_vreduce"->33,"rvv_vmask"->34,"rvv_vpermute"->35, "rvv_vamo"->36, "rvv_vmem_zvlsseg"->37,"rvv_vconfig"->38, "bitmanip_zba"->39, "bitmanip_zbb"->40, "unknown"->41) // Added RISC-V Vector functionality
    ...
    val bitmanip_zbb_inst = opstats("bitmanip_zbb").values.count(_ > 0)
    s += "--------------------------------------------------------------------------\n"
    s += "-------------- Total Bit Manipulation Zbb Instructions =  " + 24 + " --\n"
    s += "-------------- Generated Bit Manipulation Zbb Instructions =  " + bitmanip_zbb_inst + " --\n"
    s += "-------------- Bit Manipulation Zbb Instructions Coverage =  " + (bitmanip_zbb_inst.toFloat/24.toFloat)*100 + "% --\n"
...
//*****************************************************************************
  ...
  def code_body(seqnum: Int, mix: Map[String, Int], veccfg: Map[String, String], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, use_zba: Boolean, use_zbb: Boolean, segment: Boolean, rv_vmem_unit: Boolean,rv_vmem_const: Boolean,rv_vmem_vect: Boolean, rv_vmem_zvlsseg: Boolean,rv_vinteger: Boolean, rv_vfixed: Boolean, rv_vfloat: Boolean, rv_vreduce: Boolean, rv_vmask: Boolean, rv_vpermute: Boolean, rv_vamo: Boolean, rv_wide: Boolean, rv_narrow: Boolean, vlen_import: Int, lmul_import: String, sew_import: Int, nr_import: Int, nf_import: Int, mask:Boolean,  multi_config_import: Boolean, use_64bit_opcodes: Boolean) =
  {
    ...
    val name_to_seq = Map(
      ...
      "bitmanip" -> (() => new SeqBitmanip(xregs, use_zba, use_zbb, use_64bit_opcodes)),
    )
  ...
  def generate(nseqs: Int, fprnd: Int, mix: Map[String, Int], veccfg: Map[String, String], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, use_zba: Boolean, use_zbb: Boolean, segment : Boolean, loop: Boolean, loop_size: Int, rv_vmem_unit: Boolean,rv_vmem_const: Boolean,rv_vmem_vect: Boolean, rv_vmem_zvlsseg: Boolean, rv_vinteger: Boolean, rv_vfixed: Boolean, rv_vfloat: Boolean, rv_vreduce: Boolean, rv_vmask: Boolean, rv_vpermute: Boolean, rv_vamo: Boolean, rv_wide: Boolean, rv_narrow: Boolean, vlen: Int, lmul_import: String, sew_import: Int, nr_import: Int, nf_import: Int, mask :Boolean, multi_config: Boolean, use_64bit_opcodes: Boolean) =
  {
    ...
    code_body(nseqs, mix, veccfg, use_amo, use_mul, use_div, use_zba, use_zbb, segment, rv_vmem_unit, rv_vmem_const, rv_vmem_vect, rv_vmem_zvlsseg, rv_vinteger, rv_vfixed, rv_vfloat, rv_vreduce, rv_vmask, rv_vpermute, rv_vamo, rv_wide, rv_narrow, vlen, lmul, sew, nr, nf, mask, multi_config, use_64bit_opcodes) +
  ...
  def statistics(nseqs: Int, fprnd: Int, mix: Map[String, Int], vnseq: Int, vmemsize: Int, vfnum: Int, vecmix: Map[String, Int], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, use_zba: Boolean, use_zbb: Boolean, use_vec: Boolean) =
  {
    "--------------------------------------------------------------------------\n" +
